...
// Other ideas of things to implement from the book:
...
impl State {
    ...
    fn play(&mut self, ctx: &mut BTerm) {
        ...
        ctx.print(0,0, "Press SPACE to get flappy with it.");
    ...
    fn main_menu(&mut self, ctx: &mut BTerm) {
        ctx.cls();
        ctx.print_centered(5, "Welcome ot Flappy Dragon!");
...
impl Player {
    ...
    fn render(&mut self, ctx: &mut BTerm) {
        ctx.set(
            0,
            self.y,
            // Could also use RGB::from_u8() or RGB::from_hex()
            YELLOW,
            BLACK,
            to_cp437('@')
        );
...
// Using BError allows us to pass exceptions off to... the engine?
fn main() -> BError {
    let context = BTermBuilder::simple80x50()
        .with_title("Flappy Dragoncrab")
        .build()?;
